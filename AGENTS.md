...
| Agent             | Path                                        | Purpose / When to use                                                                    | Output                   |
| ----------------- | ------------------------------------------- | ---------------------------------------------------------------------------------------- | ------------------------ |
| Flow Orchestrator | `.github/agents/flow-orchestrator.agent.md` | 全体フロー統括。**入力分析 + Review Engine を内包**。チェックポイント管理。              | ハンドオフ指示、完了報告 |
| Manifest Gateway  | `.github/agents/manifest-gateway.agent.md`  | **統合 Gateway**。全入力タイプ（text/visual/portrait）に対応。早期品質チェック。         | マニフェストドラフト     |
| SVG Forge         | `.github/agents/svg-forge.agent.md`         | レビュー済みマニフェストから draw.io 編集可能な図面を生成。**自己検証 + 保存前ゲート**。 | 生成済み図面             |
