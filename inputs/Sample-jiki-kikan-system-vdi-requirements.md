...
| 項目           | 要件                                  | 備考                       |
| -------------- | ------------------------------------- | -------------------------- |
| マルチモニター | 最大 4 画面、4K 解像度対応            | RDP Shortpath 必須         |
| クリップボード | テキストのみ許可、ファイル/画像は禁止 | セキュリティポリシー準拠   |
| オーディオ     | 双方向リダイレクト有効                | Web 会議 (Teams/Zoom) 対応 |
| ビデオ         | ローカルカメラリダイレクト許可        | 会社支給端末のみ           |
| タイムゾーン   | セッション内はローカル TZ に同期      | FSLogix 設定で対応         |
| IME            | 日本語 IME をデフォルト有効化         | ゴールデンイメージで設定   |

## 29. アプリケーション互換性

| 項目                     | 対応方針                                                |
| ------------------------ | ------------------------------------------------------- |
| レガシー .NET (3.5 以前) | 互換機能パックをイメージに含める                        |
| IE モード                | Edge IE モードで対応、EMIE サイトリスト管理             |
| 32bit アプリ             | WoW64 で共存、64bit 優先                                |
| GPU 仮想化               | 初期スコープ外。CAD 利用者は別途 NVv4 シリーズを検討    |
| Java ランタイム          | Adoptium Temurin LTS を標準、旧バージョンは個別イメージ |

## 30. ライセンス管理

| ソフトウェア             | ライセンス形態                       | 備考                            |
| ------------------------ | ------------------------------------ | ------------------------------- |
| Windows 10/11 Enterprise | M365 E3/E5 (VDA 権利)                | AVD 利用権含む                  |
| Microsoft 365 Apps       | M365 E3/E5 Shared Activation         | 共有コンピューター有効化        |
| AVD                      | M365 E3/E5 or Windows E3/E5 per user | 必須                            |
| AWS WorkSpaces           | BYOL (既存 VDA 適用)                 | 従量課金は DR 時のみ            |
| 業務アプリ A             | 同時使用ライセンス 500               | ピーク時不足リスクあり → 要確認 |

## 31. ネットワーク詳細要件

| 項目                 | 設定値                                                                  | 理由                         |
| -------------------- | ----------------------------------------------------------------------- | ---------------------------- |
| MTU                  | 1400 (VPN 経由時)                                                       | IPsec オーバーヘッド考慮     |
| TCP MSS Clamping     | 1360                                                                    | フラグメント回避             |
| DNS フェイルオーバー | Azure Private DNS → オンプレ → Public                                   | SLA 順                       |
| プロキシ除外         | _.wvd.microsoft.com, _.servicebus.windows.net, \*.blob.core.windows.net | AVD 制御プレーン直接通信必須 |
| NTP                  | time.windows.com → 社内 NTP                                             | ドメイン参加時は DC 優先     |

## 32. 運用自動化

| 領域               | ツール/方式                               | 頻度            |
| ------------------ | ----------------------------------------- | --------------- |
| イメージビルド     | Packer + Azure DevOps Pipeline            | 月次 (パッチ後) |
| イメージ配布       | Azure Compute Gallery → ホストプール更新  | 月次ローリング  |
| Auto-heal          | Azure Monitor → Logic Apps → ホスト再起動 | 異常検知時即時  |
| セッションドレイン | 計画メンテ 30 分前に新規接続停止          | メンテ時        |
| 孤児セッション     | 24 時間無応答でログオフ強制               | 日次バッチ      |

## 33. サポート・エスカレーション

| レベル              | 担当                        | 対応時間                    | 連絡先                           |
| ------------------- | --------------------------- | --------------------------- | -------------------------------- |
| L1 (ユーザー問合せ) | ヘルプデスク                | 平日 8-20 時                | 内線 xxxx / helpdesk@example.com |
| L2 (技術対応)       | IT 運用チーム               | 平日 8-20 時 (P1 は 24/365) | 運用 Slack #vdi-support          |
| L3 (ベンダーエスカ) | MS Unified / AWS Enterprise | 24/365 (P1)                 | ケース起票                       |
| 緊急連絡            | PM / セキュリティリード     | 24/365                      | オンコール携帯                   |

## 34. 廃止・移行後のクリーンアップ

| 項目                 | 対応                           | 期限               |
| -------------------- | ------------------------------ | ------------------ |
| 旧 VDI 環境停止      | 並行稼働終了後にシャットダウン | 本番安定化 +30 日  |
| 旧プロファイルデータ | アーカイブ後削除               | 本番安定化 +90 日  |
| 旧ライセンス解約     | 契約更新前に見直し             | 次回更新タイミング |
| ドキュメント更新     | 運用手順書を新環境に統一       | 本番安定化 +14 日  |
